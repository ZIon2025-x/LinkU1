...
**重要提示**：现有图片不会自动迁移到新结构。需要运行迁移脚本：

```bash
# 试运行（查看将要迁移的图片）
python backend/migrate_image_structure.py --dry-run

# 执行实际迁移
python backend/migrate_image_structure.py
```
...
- 根据数据库记录确定图片类型和资源ID
- 移动到新的分类文件夹结构
- **同时更新数据库中的图片URL**，确保前端能正确访问
- 保持文件完整性，不会丢失数据

### 迁移范围

- ✅ 任务达人头像 → `expert_avatars/{expert_id}/`
- ✅ 服务图片 → `service_images/{expert_id}/`
- ✅ 任务图片 → `public/{task_id}/`
- ✅ 任务聊天私密图片 → `private_images/tasks/{task_id}/`
- ✅ 客服聊天私密图片 → `private_images/chats/{chat_id}/`
