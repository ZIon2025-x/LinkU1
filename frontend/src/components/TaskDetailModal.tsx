...
const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ isOpen, onClose, taskId }) => {
  ...
  const loadParticipants = useCallback(async () => {
    if (!task || !task.is_multi_participant || !taskId) return;
  ...
  // 当任务加载后，如果是多人任务，加载参与者列表
  useEffect(() => {
    if (task && task.is_multi_participant && taskId) {
      loadParticipants();
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontWeight: '600', color: (() => {
                    ...
                    if (isPaid) {
                      ...
                      // 如果任务已在进行中或已完成，但 is_paid 为 0，可能是使用优惠券全额抵扣或特殊情况
                      return '#94a3b8'; // 灰色（可能是优惠券全额抵扣或其他方式）
                    } else {
                    ...
                    {(() => {
                      ...
                      if (isPaid) {
                        ...
                        // 如果任务已在进行中或已完成，但 is_paid 为 0，可能是使用优惠券全额抵扣
        ...
        <div style={{
          ...
          {task.is_multi_participant && (() => {
            ...
            return isTaskManager && task.status === 'open' && (
              <button
                onClick={async () => {
      ...
      {/* 留言弹窗 */}
      {showMessageModal && selectedApplication && taskId && task && (
        <div style={{
          position: 'fixed',
          top: 0,
          left: 0,
