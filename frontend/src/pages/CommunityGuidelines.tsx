...
const CommunityGuidelines: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const doc = await getLegalDocument('community_guidelines', language);
  ...
  const getContent = (path: string) => {
    ...
    return (typeof v === 'string' ? v : null) ?? t(`communityGuidelines.${path}`);
  };

  const getSectionsFromContent = (c: Record<string, unknown>): { title: string; paragraphs: string[] }[] => {
    const sections: { title: string; paragraphs: string[] }[] = [];
    const order = ['title', 'version', 'effectiveDate', 'intro', 'contentStandards', 'forumRules', 'taskAndMarketRules', 'moderation', 'reporting', 'enforcement', 'appeals', 'updates', 'contactUs'];
    for (const key of order) {
      ...
      if (typeof v === 'string') {
        ...
        const pKeys = Object.keys(o).filter(k => k !== 'title').sort((a, b) => {
          const aNum = a.startsWith('p') ? Number(a.slice(1)) : NaN;
          const bNum = b.startsWith('p') ? Number(b.slice(1)) : NaN;
  ...
  const hasFullContent = sections.length > 3;
          ...
          <div style={{
            ...
            <div style={{ color: '#374151', fontSize: '1rem' }}>
              {hasFullContent ? (
                ...
                <>
                  <p style={{ marginBottom: '24px' }}>{getContent('intro.p1')}</p>
                  <p style={{ marginBottom: '24px' }}>{getContent('intro.p2')}</p>
                  <p style={{ margin: 0, fontSize: '0.95rem', color: '#64748b' }}>
                    {getContent('contactUs.content')}
                  </p>
                </>
              )}
            </div>
