...
// Stripe：flutter_stripe stripe_android 使用 22.2.+，stripe-connect 使用 23.0.0，
// 版本混用导致 NoClassDefFoundError: SectionFieldErrorController（uicore 与 lpm-foundations 不一致）
// 强制统一为 23.0.0
configurations.all {
    resolutionStrategy {
        force("com.stripe:stripe-android:23.0.0")
        force("com.stripe:financial-connections:23.0.0")
    }
}
...
dependencies {
    ...
    // Stripe Connect（与 flutter_stripe 12.x 共用 stripe-android 23.0）
    implementation("com.stripe:connect:23.0.0")
