"""
Customer-service domain routes — extracted from app/routers.py (Task 16,
final task of the routers.py split).

36 routes covering CS sessions, ratings, admin requests, multi-step
chat lifecycle, cancel-request reviews, file uploads, and timeout
management.

This is the last domain extraction. After this commit:
  - app/routers.py contains zero @router decorators (helpers only).
  - main.py no longer mounts main_router; all 152 routes go through
    _SPLIT_ROUTERS plus the per-domain admin_*_routes.py modules.
