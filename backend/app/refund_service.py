...
def process_package_refund(
    db: Session,
    refund_request: models.RefundRequest,
    refund_amount: float
) -> Tuple[bool, Optional[str], Optional[str], Optional[str]]:
    """
    处理套餐退款（RefundRequest.package_id 不为 NULL）

    Args:
        db: 数据库会话
        refund_request: 退款申请记录（package_id 必须已设置）
        refund_amount: 退款金额（英镑）
    ...
    Returns:
        ...
        - success: 是否成功
        - refund_intent_id: Stripe Refund ID（如果有）
        - refund_transfer_id: 反向转账 ID（套餐退款不涉及，固定为 None）
        - error_message: 错误信息（如果失败）
    """
    try:
        if not stripe.api_key:
        ...
        if not refund_request.package_id:
            return False, None, None, "refund_request.package_id 未设置，无法走套餐退款分支"

        # ── Step 1: 解析 UserServicePackage ────────────────────────────────
        ...
        if not pkg:
            logger.error(
                f"RefundRequest {refund_request.id} 引用了不存在的套餐 "
                f"{refund_request.package_id}"
            )
            refund_request.status = "failed"
            refund_request.last_error = "package_not_found"
            return False, None, None, "package_not_found"

        if not pkg.payment_intent_id:
            logger.error(
                f"RefundRequest {refund_request.id} 的套餐 {refund_request.package_id} "
                f"缺少 payment_intent_id，无法发起 Stripe 退款"
            )
            refund_request.status = "failed"
            refund_request.last_error = "package_payment_intent_missing"
            return False, None, None, "package_payment_intent_missing"
        ...
        try:
            ...
            if payment_intent.status == "canceled":
                logger.warning(
                    f"PaymentIntent {payment_intent_id} 已取消，套餐无需退款"
                )
            else:
                refund_amount_decimal = Decimal(str(refund_amount))
                refund_amount_pence = int(refund_amount_decimal * 100)
                ...
                if charges.data:
                    ...
                    idempotency_key = hashlib.sha256(
                        f"pkg_refund_{refund_request.package_id}_{refund_request.id}_{refund_amount_pence}".encode()
                    ).hexdigest()
                    ...
                    refund = stripe.Refund.create(
                        ...
                        idempotency_key=idempotency_key,
                        metadata={
                            "package_id": str(refund_request.package_id),
                            "refund_request_id": str(refund_request.id),
                    ...
                    refund_intent_id = refund.id
                    logger.info(
                        f"✅ 套餐 Stripe 退款创建成功: refund_id={refund.id}, "
                        f"package_id={refund_request.package_id}, amount=£{refund_amount:.2f}"
                    )
                else:
                    logger.warning(
                        f"PaymentIntent {payment_intent_id} 没有找到 Charge，"
                        f"套餐退款无法继续"
                    )
                    return False, None, None, "stripe_charge_not_found"

        except stripe.error.StripeError as e:
            logger.error(f"套餐 Stripe 退款失败: {e}")
            return False, None, None, f"Stripe 退款失败: {str(e)}"

        # ── Step 3: 更新 UserServicePackage 退款字段 ────────────────────────
        # 重新查询（加锁），确保写入最新版本
        from sqlalchemy import select as sa_select
        locked_pkg_query = sa_select(UserServicePackage).where(
            UserServicePackage.id == refund_request.package_id
        ).with_for_update()
        pkg = db.execute(locked_pkg_query).scalar_one_or_none()
        if pkg:
            pkg.refunded_at = get_utc_time()
            pkg.refunded_amount_pence = int(Decimal(str(refund_amount)) * 100)
            # ❌ 不修改 pkg.status — refunded / partially_refunded 是终态，
            #    由调用退款端点时同步写入，此处不得覆盖。
            logger.info(
                f"✅ UserServicePackage {refund_request.package_id} 退款字段已更新: "
                f"refunded_amount_pence={pkg.refunded_amount_pence}, "
                f"refunded_at={pkg.refunded_at}"
            )

        return True, refund_intent_id, None, None

    except Exception as e:
        logger.error(f"处理套餐退款时发生错误: {e}", exc_info=True)
        return False, None, None, f"处理套餐退款时发生错误: {str(e)}"
