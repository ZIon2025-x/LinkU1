...
def store_verification_code(email: str, code: str) -> bool:
    ...
    try:
        ...
        key = f"{CODE_KEY_PREFIX}:{email}"
        code_data = {
            "code": code,
            "email": email,
            "created_at": datetime.utcnow().isoformat(),
            "expires_at": (datetime.utcnow() + timedelta(seconds=VERIFICATION_CODE_EXPIRE_SECONDS)).isoformat()
        }
        ...
        logger.info(f"验证码已存储: email={email}, code={code}")
        return True
...
def verify_and_delete_code(email: str, code: str) -> bool:
    """验证验证码，验证成功后自动删除"""
    try:
        redis_client = get_redis_client()
        if not redis_client:
        ...
        # 验证验证码
        if stored_code != code:
            logger.warning(f"验证码不匹配: email={email}, provided={code}, stored={stored_code}")
            return False
        
        # 验证成功，删除验证码
        redis_client.delete(key)
        logger.info(f"验证码验证成功并已删除: email={email}")
        return True
        
    except json.JSONDecodeError as e:
        logger.error(f"解析验证码数据失败: {e}")
        # 清理无效数据
        try:
            redis_client = get_redis_client()
            if redis_client:
