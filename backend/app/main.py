...
# Routes have been migrated out of routers.py into app/routes/<domain>_routes.py
# (see _SPLIT_ROUTERS below) and per-domain admin_*_routes.py modules
# (registered individually below). The old `from app.routers import router as
# main_router` is gone — routers.py no longer exposes a router object.
...
# main_router /api/users mount removed — see _SPLIT_ROUTERS loop below for
# the equivalent dual-mount setup over the per-domain routers.

# 客服端员工通知（/api/users/staff/notifications*），保留 /api/users 单挂载
...
# AI 限时问答 — Admin 端路由 (/api/admin/ai-qa/*); prefix 已在 APIRouter 里声明，此处不重复
...
# 任务达人功能路由 — Phase B1 收口第四轮 (2026-04-09) 已删除
# 原 task_expert_routes.py 所有 buyer 端点迁移到:
