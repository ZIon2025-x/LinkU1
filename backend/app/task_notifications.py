...
def send_task_application_notification(
    ...
    try:
        ...
        # 发送邮件
        email_subject = f"Link²Ur - 新任务申请：{task.title}"
        email_body = f"""
        <html>
...
def send_task_approval_notification(
    ...
    try:
        ...
        # 发送邮件
        email_subject = f"Link²Ur - 任务申请已同意：{task.title}"
        email_body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
...
def send_task_completion_notification(
    ...
    try:
        ...
        # 发送邮件
        email_subject = f"Link²Ur - 任务已完成：{task.title}"
        email_body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
...
def send_task_confirmation_notification(
    ...
    try:
        ...
        # 发送邮件
        email_subject = f"Link²Ur - 任务已确认完成：{task.title}"
        email_body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
...
def send_task_rejection_notification(
    ...
    try:
        ...
        # 发送邮件
        email_subject = f"Link²Ur - 任务申请被拒绝：{task.title}"
        email_body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
