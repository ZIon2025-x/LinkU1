-- 迁移 224：给 refund_policy 13 个 content section 加双语标题
-- 依赖：223（refund_policy 行已存在）
--
-- 问题：223 把每个 content section 存成裸字符串，前端 LegalDocument 解析后
-- LegalSection.title 落到英文 key 名（如 "intro"、"fullRefund"），用户看到的章节
-- 标题没本地化。
--
-- 修法：每个 content section 改成 {"title": "...", "content": "..."} 对象，
-- 模型读到 Map 时会用内层 title 作为章节标题（详见 link2ur/lib/data/models/
-- legal_document.dart 第 44-60 行：value is Map → 取 title key）。
-- 元数据 keys (lastUpdated/version/effectiveDate) 保持字符串形态，由前端
-- _MetadataStrip 用 _metadataKeys 过滤后单独渲染。
--
-- 幂等：使用 UPDATE，可多次执行。

-- ========== 中文 (zh) ==========
UPDATE legal_documents SET content_json = $REFUND_ZH_V2$
{
  "title": "退款政策",
  ...
  "eligibility": {
    "title": "申请退款的条件",
    "content": "退款申请须满足以下条件之一：任务尚未确认完成（状态为「待确认」之前）；双方协商一致取消；服务方违反平台规则被处理；平台原因导致服务无法继续。一旦任务双方确认完成，原则上不可申请退款。"
  },
  "fullRefund": {
    "title": "可全额退款的情形",
  ...
  "partialRefund": {
    "title": "按比例退款的情形",
    "content": "以下情形可申请按比例退款：（1）服务方已部分交付，任务部分完成；（2）双方就部分履约达成协议；（3）因发布方原因中止但服务方已投入工作。退款比例（0%–100%）由双方协商或平台审核裁定，通常按已完成工作量、已发生成本、约定服务标准等因素综合判定。"
  },
  "nonRefundable": {
    "title": "不予退款的情形",
    "content": "以下情形不予退款：（1）任务已确认完成且双方均已评价；（2）服务方按约履约后，因发布方个人原因取消；（3）提供虚假证据或恶意申请退款；（4）申请超出本政策规定的有效期或任务状态窗口。"
  },
  "refundProcess": {
    "title": "退款流程",
    "content": "退款流程：（1）任务发布方在任务详情页选择「申请退款」；（2）选择退款类型（全额/按比例）、退款原因、退款金额；（3）服务方在 48 小时内回应（同意/异议）；（4）双方达成一致后退款进入处理；（5）双方未达成一致时申请平台介入，客服在 3-5 个工作日内裁定；（6）审核通过后按原支付路径退款。"
  },
  "refundTime": {
    "title": "到账时效",
  ...
  "walletAndCoupon": {
    "title": "钱包与优惠券处理",
    "content": "钱包与优惠券处理：（1）使用钱包余额抵扣的部分，退款时退回钱包；Stripe 渠道支付的部分，退回原卡。（2）全额退款时，使用过的优惠券将返还到您的账户（可在有效期内重复使用）；按比例退款时，优惠券视为已使用，不予返还。（3）积分抵扣的处理同优惠券规则。"
  },
  "disputeResolution": {
    "title": "争议处理",
    "content": "争议处理：双方协商不成时，任意一方可申请平台介入。客服将在 3-5 个工作日内基于聊天记录、任务进度证据、双方陈述等综合裁定。平台裁定为最终结果，双方应予执行。涉及金额较大或复杂争议的，平台可延长审核期并要求补充证据。"
  },
  "vipSubscription": {
    "title": "VIP 订阅退款",
...
-- ========== English (en) ==========
UPDATE legal_documents SET content_json = $REFUND_EN_V2$
{
  "title": "Refund Policy",
  ...
  "walletAndCoupon": {
    ...
    "content": "Wallet and coupon handling: (1) the portion paid by wallet balance is refunded to the wallet; the portion paid via Stripe is refunded to the original card. (2) On a full refund, any coupon used will be returned to your account and may be re-used within its validity; on a pro-rata refund, the coupon is treated as used and will not be returned. (3) Points used for deduction follow the same rules as coupons."
  },
  "disputeResolution": {
...
-- 验证
-- SELECT type, lang, jsonb_typeof(content_json->'intro') FROM legal_documents WHERE type='refund_policy';
-- 期望：2 行，jsonb_typeof = 'object' (而不是 'string')
