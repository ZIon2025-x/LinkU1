...
**注意**：之前的迁移脚本已经执行过，不再重复执行。当前只包含新的性能优化索引迁移。

### 当前迁移脚本

**数据库性能优化索引迁移** (`migrations/add_performance_indexes.sql`)
   - 启用 pg_trgm 扩展（用于相似度搜索）
   - 创建任务表复合索引（游标分页、组合查询、用户查询）
   - 创建申请表索引（申请者查询、任务申请状态）
   - 创建消息表索引（对话查询、接收者查询）
   - 创建通知表索引（用户通知查询）
   - 创建 pg_trgm 索引（相似度搜索）
   - 创建全文搜索索引（可选）
   - 添加 conversation_key 字段和触发器（优化消息对话查询）
...
- `backend/migrations/add_performance_indexes.sql` - 数据库性能优化索引迁移脚本（**新增**）
