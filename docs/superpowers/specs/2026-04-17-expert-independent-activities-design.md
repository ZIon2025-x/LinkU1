...
draw_at: Optional[datetime] = None     # 自动开奖时间（lottery + auto only）
```
...
Activity 模型赋值时，新增字段直接写入（DB 列已存在，无需迁移）：
...
```python
# 原逻辑：只扫官方账号的 lottery 活动
# 新逻辑：扫所有 lottery + auto + 未开奖 + 到期的活动
where(
...
- 校验：`activity_type == 'lottery'`，`draw_mode == 'manual'`，`is_drawn == False`
...
**无需 migration** — `activities` 表已有所有需要的列：
