...
|------|------|------|
| id | INT PK | 自增主键 |
| type | VARCHAR(20) NOT NULL DEFAULT 'normal' | 群组类型：`'normal'`（普通群聊）/ `'expert_internal'`（达人内部群聊） |
...
-- 群组发现（排除 expert_internal）
CREATE INDEX ix_groups_category ON chat_groups(category) WHERE is_active = true AND type = 'normal';
CREATE INDEX ix_groups_tags ON chat_groups USING gin(tags) WHERE type = 'normal';
CREATE INDEX ix_groups_name_trgm ON chat_groups USING gin(name gin_trgm_ops) WHERE type = 'normal';
...
| GET | `/api/groups` | 发现群组（搜索用 trgm 模糊匹配/分类/标签，**自动排除 `expert_internal`**） | 登录用户 |
| GET | `/api/groups/my` | 我的群组列表（含 `normal` + `expert_internal`） | 登录用户 |
...
├── data/
│   ├── models/
│   │   ├── chat_group.dart          # ChatGroup (含 type: normal/expert_internal), GroupMember, JoinRequest, GroupInvitation
...
```sql
-- 用 CTE 一次性创建内部群聊 + 关联 + 同步成员
WITH new_groups AS (
    INSERT INTO chat_groups (type, name, owner_id, join_mode, max_members, member_count, created_at, updated_at)
    SELECT
        'expert_internal',
        e.name || ' 内部群',
        em.user_id,
...
link_experts AS (
    UPDATE experts e
    SET internal_group_id = ng.id
    FROM new_groups ng
    JOIN expert_members em ON em.user_id = ng.owner_id AND em.role = 'owner'
    WHERE em.expert_id = e.id
    RETURNING e.id AS expert_id, e.internal_group_id AS group_id
)
-- 同步所有活跃成员到群聊
...
SELECT
    ...
    'active',
    NOW()
FROM link_experts le
JOIN expert_members em ON em.expert_id = le.expert_id AND em.status = 'active'
...
| 团队事件 | 群聊操作 | 端点 | 说明 |
|---------|---------|------|------|
| 用户加入团队 | `INSERT/UPDATE group_members (status='active')` | `respond_to_invitation`, `review_join_request` | 重新加入时 UPDATE |
| 用户退出团队 | `UPDATE group_members SET status='left'` | `leave_team` | |
| 用户被移除 | `UPDATE group_members SET status='banned'` | `remove_member` | expert_members.status='removed' → group_members.status='banned' |
...
| 团队名称变更 | `UPDATE chat_groups SET name = new_name || ' 内部群'` | `PUT /api/experts/{id}` | 群名跟随团队名 |
| 团队解散 | `UPDATE chat_groups SET is_active=false` | `dissolve_expert_team` | 软删除，消息保留 |

> **Status 映射**：`expert_members.status` 值为 `'active'/'left'/'removed'`，`group_members.status` 值为 `'active'/'left'/'banned'`。映射关系：`removed` → `banned`。
>
> **建议**：封装 `sync_expert_group_member()` 辅助函数，所有成员变动端点统一调用，避免未来新增端点时遗漏同步。
...
| 在群组发现中 | 显示 | **不显示**（`GET /api/groups` 自动排除） |

### chat_groups 表扩展

`chat_groups.type` 已在 Phase 1 建表时包含（见数据模型章节），值为 `'normal'` 或 `'expert_internal'`。无需额外迁移。
...
2. `ExpertTeam` 模型需新增 `internalGroupId` 字段（`int?`），`fromJson` 加 `json['internal_group_id']`
