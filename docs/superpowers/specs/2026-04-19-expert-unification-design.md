...
| Phase | 范围 | 前置 | 产出 |
|-------|------|------|------|
| **A** | catch-up migration + 11 处 TaskExpert + 10 处 FeaturedTaskExpert 引用切到新模型。对外 API 返回 schema 保持不变（内部实现 swap），admin panel 继续工作 | — | 一个大 PR；后端单写新表；老表仍存在但只读 |
...
1. `grep -r "models\.TaskExpert\b\|models\.FeaturedTaskExpert" backend/app/` 结果仅保留 `models.py` / `admin_task_expert_routes.py` / `crud/task_expert.py`（Phase B/C 范围）
...
| 文件 | 行号 | 当前逻辑 | 改造 |
|------|------|----------|------|
| `routers.py` | 4798-4808 | `TaskExpert OR ExpertMember` 双查 | 替换为 `is_user_expert_sync(db, current_user.id)` |
| `routers.py` | 5085-5093 | 同上 | 替换为 `is_user_expert_sync(db, user_id)` |
| `admin_official_routes.py` | 58-69 (`_get_official_expert`) | `TaskExpert.is_official=True` | 改查 `Expert.is_official=True` |
| `admin_official_routes.py` | 195-219 (`get_official_account`) | JOIN TaskExpert + User | 查 Expert + ExpertMember(owner) JOIN User，**返回 schema 不变** |
| `discovery_routes.py` | 747-792 (`_fetch_expert_services`) | JOIN `TaskExpertService.expert_id → TaskExpert` | JOIN `owner_type='expert' + owner_id → Expert` |
| `follow_feed_routes.py` | 421, 569 | FeaturedTaskExpert 展示 | 切 FeaturedExpertV2 JOIN Expert |
| `multi_participant_routes.py` | 1604, 1609, 1872 | `TaskExpert.id == user_id` 检查 | 替换为 `is_user_expert_sync()` |
...
| `crud/user.py:80-110` 统计同步 | 写 User + TaskExpert + FeaturedTaskExpert | 写 User + Expert（通过 `_expert_id_migration_map` 查 new_id）；不再写 FeaturedExpertV2（其 schema 精简后只含 is_featured/display_order/category，由 admin_expert_routes 的 admin 操作驱动写入，Phase B 完整迁移） |
| `crud/task_expert.py:141-159` 聚合 success_rate | 写 FeaturedTaskExpert.success_rate | 改写 `Expert.success_rate`（通过映射查 new_id） |
| `crud/admin_ops.py:73-74` 删 admin 时检查 FeaturedTaskExpert | 读 FeaturedTaskExpert | 改读 FeaturedExpertV2 |

### 7.4 边缘改造

| 文件 | 改造 |
|------|------|
| `cleanup_tasks.py:1158-1425` 图片清理 | 遍历 `ExpertMember where role='owner'` 的 user_id，语义等价 |
| `main.py:1252` ORM 初始化 import | 保留（模型 Phase C 删） |
| `admin_task_expert_routes.py:368` 注释 | 不动（Phase B 整个文件删） |
...
| R2 | `is_user_expert_sync` 漏判老 TaskExpert 孤儿行 | 高（用户失去达人入口） | Phase A PR merge 前验证 migration 185 `orphan_count=0` |
...
| R9 | 大 PR 审查疲劳 | 中 | PR 按 §7 的 6 类改造分 section，reviewer 可分块审 |
