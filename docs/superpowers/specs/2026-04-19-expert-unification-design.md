...
1. **Phase A 范围的文件**零 `TaskExpert` / `FeaturedTaskExpert` 引用（详见 §7.2-7.4）；Phase B/C 范围的文件（详见 §7.5）保留引用 — 它们在 Phase B / Phase C 处理
...
| 文件 | 行号 | 当前逻辑 | 改造 |
|------|------|----------|------|
| `routers.py` | 4781-4785, 4804-4808 (`get_my_profile`) | `TaskExpert OR ExpertMember` 双查 | `is_user_expert_sync(db, current_user.id)` |
| `routers.py` | 5072-5076, 5089-5093 (`get_user_profile`) | 同上 | `is_user_expert_sync(db, user_id)` |
...
| `crud/task_expert.py` | 141-165 (聚合 success_rate + update_task_expert_featured_response_time) | 写 FeaturedTaskExpert.success_rate/response_time | 改写 `Expert.success_rate` 和 `Expert.response_time`（通过映射查 new_id） |
| `crud/task_expert.py` | 172-179 (`update_all_response_times` 定时任务) | 遍历 FeaturedTaskExpert 更新响应时间 | 改遍历 Expert 表（或确认该函数未被调度则注明并留到 Phase C 删）— **writing-plans 阶段需先 grep 调用点确认死活** |
...
```bash
# 改动边界验证:以下命令返回的文件列表应该 **只** 包含 §7.6 列出的 Phase B/C 文件
grep -rn "\bmodels\.TaskExpert\b\|\bmodels\.FeaturedTaskExpert\b\|from app\.models import.*TaskExpert" backend/app/
```
