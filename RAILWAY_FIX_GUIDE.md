...
现在包含三层防护：
1. 使用 `pg_class` 系统表查询并删除所有索引对象
2. **使用 raw psycopg2 + autocommit**：绕过 SQLAlchemy 事务层，确保 DROP 立即生效
3. **引擎连接池重置**：删除对象后调用 `engine.dispose()` 确保新连接可见
4. **智能重试机制**：如果创建失败并报"已存在"错误，自动删除孤立对象并重试（最多3次）
