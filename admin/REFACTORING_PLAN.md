...
- ✅ 优惠券表单 UI 优化（添加折叠分组）
- ✅ 创建目录结构
- ✅ **阶段 1: 创建共享组件和 hooks（已完成）**
  - ✅ `useAdminTable` - 通用表格逻辑
  - ✅ `useModalForm` - 模态框表单逻辑
  - ✅ `useAdminApi` - API 调用逻辑
  - ✅ `AdminTable` - 通用表格组件
  - ✅ `AdminModal` - 通用模态框组件
  - ✅ `AdminPagination` - 分页组件
  - ✅ `StatusBadge` - 状态标签组件
- ✅ **优惠券管理模块完整重构（作为示例）**
  - ✅ `CouponManagement.refactored.tsx` - 使用新架构
  - ✅ `CouponFormModal.tsx` - 完整表单组件
  - ✅ 类型定义和样式文件
- ✅ **AdminLayout 框架（已完成）**
  - ✅ 响应式侧边栏
  - ✅ 顶部栏和用户菜单
...
### 🎯 下一步（按优先级）
1. **测试优惠券模块** - 确保重构后的模块正常工作
2. **逐步提取其他模块** - 参考优惠券模块的实现方式
   - 用户管理
   - 专家管理
   - 纠纷管理
   - 退款管理
