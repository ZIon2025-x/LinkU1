...
class APIService {
                ...
                if (200...299).contains(httpResponse.statusCode) {
                    ...
                    if let urlString = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines),
                       urlString.hasPrefix("http") {
                        return Just(urlString).eraseToAnyPublisher()
                    ...
                    // Token过期，尝试刷新
                    return self.handle401Error()
                        .flatMap { _ -> AnyPublisher<String, APIError> in
                            // 重试上传
    ...
    // 注册设备Token（用于推送通知）
    func registerDeviceToken(_ token: String, completion: @escaping (Bool) -> Void) {
