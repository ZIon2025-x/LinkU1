# App Store 上架元数据

## 📱 应用基本信息

**应用名称**：Link²Ur  
**Bundle ID**：com.link2ur  
**版本**：1.0  
**支持网站**：https://www.link2ur.com  
**技术支持网站**：https://www.link2ur.com/support  
**隐私政策**：https://www.link2ur.com/privacy  
**服务条款**：https://www.link2ur.com/terms  
**联系邮箱**：info@link2ur.com  
**联系地址**：A3, B16 9NB, UK

---

## 🇨🇳 中文版本

### 推广文本（Promotional Text）
**最多 170 个字符，可随时更新**

```
🎓 专为学生打造的本地服务平台！发布任务、买卖二手、参与社区，连接身边的技能与需求。安全可靠，立即体验！
```

### 简短描述（Subtitle）
**最多 30 个字符**

```
连接学生与本地服务的平台
```

### 关键词（Keywords）
**最多 100 个字符，用逗号分隔，无空格**

```
link2ur,link to you,link2u,学生,任务,服务,跳蚤市场,论坛,社区,本地服务,兼职,二手,交易,任务平台,学生服务,校园服务,技能匹配,任务发布,二手交易,社区论坛
```

### 完整描述（Description）
**最多 4000 个字符**

```
Link²Ur 是一个专为学生设计的本地服务平台，帮助您轻松连接身边的技能与需求。

🌟 核心功能

• 任务市场
  发布任务，找到合适的帮手；接受任务，赚取额外收入。无论是学习辅导、搬家协助，还是其他日常需求，都能在这里找到解决方案。

• 跳蚤市场
  浏览和购买二手商品，或发布自己的闲置物品。从教科书到家具，从电子产品到生活用品，应有尽有。

• 社区论坛
  参与讨论，分享经验，结识新朋友。按大学分类，找到属于你的社区。

• 实时消息
  与任务发布者、商品卖家实时聊天，快速沟通，高效交易。

• 个人中心
  管理您的任务、订单、钱包和偏好设置，一切尽在掌握。

🔒 安全可靠

• 学生身份验证，确保平台用户真实性
• 安全的支付系统，保障交易安全
• 完善的隐私保护，保护您的个人信息
• 专业的客服支持，随时为您服务

🎯 适用场景

• 需要帮助完成某项任务
• 想要通过技能赚取收入
• 寻找或出售二手商品
• 参与校园社区讨论
• 连接本地服务提供者

立即下载 Link²Ur，开启您的本地服务之旅！

服务条款（Terms of Use）：https://www.link2ur.com/terms
```

**Guideline 3.1.2 合规**：提供自动续期订阅的应用须在 App 描述或 EULA 栏提供可用的 Terms of Use 链接。请务必在描述末尾保留上述「服务条款」链接一行，或在 App Store Connect 的 EULA 处填写自定义 EULA。

### 版权信息（Copyright）
```
© 2024 Link²Ur. All rights reserved.
```

---

## 🇬🇧 英文版本

### Promotional Text
**Maximum 170 characters, can be updated anytime**

```
🎓 The local service platform designed for students! Publish tasks, buy & sell items, join communities. Connect skills with needs. Safe & reliable. Download now!
```

### Subtitle
**Maximum 30 characters**

```
Connect Students with Local Services
```

### Keywords
**Maximum 100 characters, comma-separated, no spaces**

```
link2ur,link to you,link2u,student,task,service,flea market,forum,community,local service,part-time,secondhand,trade,task platform,student service,campus service,skill matching,task publishing,secondhand trading,community forum
```

### Description
**Maximum 4000 characters**

```
Link²Ur is a local service platform designed specifically for students, helping you easily connect skills with needs in your community.

🌟 Core Features

• Task Marketplace
  Publish tasks to find the right help, or accept tasks to earn extra income. Whether it's tutoring, moving assistance, or other daily needs, you can find solutions here.

• Flea Market
  Browse and buy secondhand items, or list your own unused items. From textbooks to furniture, from electronics to daily necessities, everything is available.

• Community Forum
  Join discussions, share experiences, and make new friends. Organized by universities, find your community.

• Real-time Messaging
  Chat with task posters and sellers in real-time for quick communication and efficient transactions.

• Personal Center
  Manage your tasks, orders, wallet, and preferences - everything at your fingertips.

🔒 Safe & Reliable

• Student identity verification ensures authentic platform users
• Secure payment system protects your transactions
• Comprehensive privacy protection safeguards your personal information
• Professional customer support available anytime

🎯 Perfect For

• Need help completing a task
• Want to earn income through your skills
• Looking for or selling secondhand items
• Participating in campus community discussions
• Connecting with local service providers

Download Link²Ur now and start your local service journey!

Terms of Use: https://www.link2ur.com/terms
```

**Guideline 3.1.2 compliance**: Apps offering auto-renewable subscriptions must include a functional link to Terms of Use (EULA) in the App Description or in the EULA field in App Store Connect. Keep the "Terms of Use" line above at the end of the description, or provide a custom EULA in the EULA field.

### Copyright
```
© 2024 Link²Ur. All rights reserved.
```

---

## 📋 App Store Connect 填写指南

### 1. 应用信息（App Information）
- **名称**：Link²Ur
- **副标题**：使用上面的"简短描述"
- **类别**：商业（Business）或 社交（Social Networking）
- **内容版权**：使用上面的"版权信息"

### 2. 定价和可用性（Pricing and Availability）
- **价格**：免费（Free）
- **可用地区**：根据实际情况选择

### 3. 应用隐私（App Privacy）
- **数据收集**：根据实际功能填写
- **隐私政策 URL**：https://www.link2ur.com/privacy（必填，且须可访问）

### 3.1 Guideline 3.1.2 - 订阅应用 EULA（必填）
- **Terms of Use (EULA)**：在「版本信息」的**描述**末尾加入可点击链接「服务条款：https://www.link2ur.com/terms」（见上方完整描述示例），或在 App Store Connect 的 **EULA / 许可协议** 处选择「自定义 EULA」并填写条款。

### 4. 版本信息（Version Information）
- **推广文本**：使用上面的"推广文本"
- **描述**：使用上面的"完整描述"
- **关键词**：使用上面的"关键词"
- **支持 URL**：https://www.link2ur.com/support
- **营销 URL**：https://www.link2ur.com

### 5. 应用审核信息（App Review Information）
- **联系信息**：info@link2ur.com
- **演示账户**：提供测试账户信息（如需要）
- **备注**：说明应用的主要功能和测试方法

### 6. 版本发布（Version Release）
- **自动发布**：审核通过后自动发布
- **手动发布**：审核通过后手动发布（推荐首次上架）

---

## ⚠️ 重要提示

1. **关键词必须包含 "link2ur"**：确保用户搜索应用名称时能找到
2. **描述要突出核心功能**：任务、跳蚤市场、论坛
3. **强调安全性**：学生验证、安全支付、隐私保护
4. **使用表情符号**：让描述更吸引人（但不要过度使用）
5. **定期更新推广文本**：可以随时更新，用于推广活动

---

## 📝 检查清单

- [ ] 所有文本长度符合要求
- [ ] 关键词包含 "link2ur"
- [ ] 所有 URL 可访问
- [ ] 版权信息正确
- [ ] 中英文版本都已准备
- [ ] 联系信息准确
- [ ] 隐私政策和服务条款已准备
- [ ] **Guideline 3.1.2**：描述末尾含可点击的 Terms of Use 链接，或已在 EULA 栏填写自定义 EULA；应用内 VIP 购买页已提供隐私政策与服务条款链接

---

**最后更新**：2024年1月
