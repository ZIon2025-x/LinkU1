# LinkU 数据库优化效果检查指南

## 📋 概述

本文档提供详细的检查方法，用于验证数据库优化是否成功实施。

**检查原则**:
- ✅ 使用数据说话（量化指标）
- ✅ 对比优化前后的性能
- ✅ 验证索引是否真正被使用
- ✅ 确认没有引入新的问题

---

## 🔍 检查方法总览

### 快速检查清单

- [ ] **性能指标对比**：响应时间、QPS、数据库负载
- [ ] **N+1 查询检测**：确认查询次数合理
- [ ] **索引使用验证**：使用 EXPLAIN ANALYZE 确认索引生效
- [ ] **缓存效果检查**：缓存命中率、缓存大小
- [ ] **功能正确性**：数据准确性、分页正确性

---

## 1. 性能指标对比

### 1.1 响应时间对比

**检查方法**:

#### 方法A：使用应用日志（推荐）

在关键接口添加性能日志：

```python
# backend/app/async_routers.py
import time
import logging

logger = logging.getLogger(__name__)

@async_router.get("/tasks")
async def get_tasks(...):
    start_time = time.time()
    try:
        # ... 查询逻辑 ...
        result = await async_crud.async_task_crud.get_tasks_with_total(...)
        elapsed = (time.time() - start_time) * 1000  # 转换为毫秒
        
        # 记录性能指标
        logger.info(
            f"GET /tasks - elapsed: {elapsed:.2f}ms, "
            f"task_count: {len(result[0])}, "
            f"total: {result[1]}"
        )
        
        return result
    except Exception as e:
        elapsed = (time.time() - start_time) * 1000
        logger.error(f"GET /tasks - error after {elapsed:.2f}ms: {e}")
        raise
```

**检查步骤**:

1. **收集优化前的数据**（如果还没有，可以临时添加日志运行一段时间）
   ```bash
   # 查看日志中的响应时间
   grep "GET /tasks" app.log | awk '{print $NF}' | sort -n
   ```

2. **收集优化后的数据**
   ```bash
   # 同样的命令
   grep "GET /tasks" app.log | awk '{print $NF}' | sort -n
   ```

3. **对比分析**
   ```python
   # 计算统计指标
   # 优化前：平均 300ms，P95: 500ms，P99: 800ms
   # 优化后：平均 80ms，P95: 150ms，P99: 250ms
   # 提升：平均 3.75倍，P95: 3.33倍，P99: 3.2倍
   ```

#### 方法B：使用 APM 工具（如 New Relic、Datadog）

如果项目已集成 APM 工具，可以直接查看：
- 平均响应时间
- P95/P99 响应时间
- 请求量（QPS）
- 错误率

#### 方法C：使用压测工具（推荐用于验证）

使用 **Locust** 或 **k6** 进行压测：

```python
# locustfile.py
from locust import HttpUser, task, between
import time

class TaskListUser(HttpUser):
    wait_time = between(1, 3)
    
    @task
    def get_tasks(self):
        start = time.time()
        response = self.client.get("/tasks?limit=20&sort_by=latest")
        elapsed = (time.time() - start) * 1000
        
        # 记录响应时间
        if response.status_code == 200:
            print(f"Response time: {elapsed:.2f}ms")
        else:
            print(f"Error: {response.status_code}, time: {elapsed:.2f}ms")
```

**运行压测**:
```bash
# 启动 Locust
locust -f locustfile.py --host=http://localhost:8000

# 访问 http://localhost:8089 进行压测
# 设置：100 并发用户，持续 5 分钟
```

**检查指标**:
- 平均响应时间（应该 < 100ms）
- P95 响应时间（应该 < 200ms）
- 失败率（应该 < 0.1%）
- RPS（每秒请求数，应该 > 50）

### 1.2 数据库负载对比

**检查方法**:

#### 查看 PostgreSQL 统计信息

```sql
-- 1. 查看查询执行时间统计（需要启用 pg_stat_statements）
SELECT 
    query,
    calls,
    total_exec_time::numeric / 1000 as total_seconds,
    mean_exec_time::numeric / 1000 as mean_seconds,
    max_exec_time::numeric / 1000 as max_seconds
FROM pg_stat_statements
WHERE query LIKE '%tasks%'
ORDER BY total_exec_time DESC
LIMIT 10;

-- 2. 查看数据库连接数
SELECT count(*) FROM pg_stat_activity;

-- 3. 查看慢查询（执行时间 > 100ms）
SELECT 
    query,
    calls,
    mean_exec_time::numeric / 1000 as mean_seconds
FROM pg_stat_statements
WHERE mean_exec_time > 100000  -- 100ms in microseconds
ORDER BY mean_exec_time DESC
LIMIT 20;
```

**优化前后对比**:
- 查询执行时间应该降低 50-80%
- 慢查询数量应该显著减少
- 数据库连接数应该保持稳定

---

## 2. N+1 查询检测

### 2.1 使用 SQLAlchemy 日志

**启用 SQL 日志**:

```python
# backend/app/database.py
import logging

# 启用 SQLAlchemy 日志
logging.getLogger('sqlalchemy.engine').setLevel(logging.INFO)

# 或者在创建引擎时设置
engine = create_async_engine(
    DATABASE_URL,
    echo=True,  # 打印所有 SQL 查询
    echo_pool=True  # 打印连接池信息
)
```

**检查方法**:

1. **触发一个请求**（例如：获取用户申请列表）
   ```bash
   curl -X GET "http://localhost:8000/my-applications" \
     -H "Cookie: session_id=xxx"
   ```

2. **查看日志中的 SQL 查询**
   ```bash
   # 应该看到类似这样的日志
   # 优化前（N+1 查询）:
   # SELECT * FROM task_applications WHERE applicant_id = '123'  -- 1 次
   # SELECT * FROM tasks WHERE id = 1  -- N 次（每个申请一次）
   # SELECT * FROM tasks WHERE id = 2  -- N 次
   # ... (总共 N+1 次查询)
   
   # 优化后（批量查询）:
   # SELECT * FROM task_applications WHERE applicant_id = '123'  -- 1 次
   # SELECT * FROM tasks WHERE id IN (1, 2, 3, ...)  -- 1 次（批量查询）
   # (总共 2 次查询)
   ```

3. **统计查询次数**
   ```python
   # 使用 SQLAlchemy 事件监听器
   from sqlalchemy import event
   from sqlalchemy.engine import Engine
   
   query_count = 0
   
   @event.listens_for(Engine, "before_cursor_execute")
   def receive_before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
       global query_count
       query_count += 1
       print(f"Query #{query_count}: {statement[:100]}...")
   
   # 在请求处理函数中重置计数器
   @async_router.get("/my-applications")
   async def get_user_applications(...):
       global query_count
       query_count = 0
       
       # ... 查询逻辑 ...
       
       print(f"Total queries: {query_count}")
       return result
   ```

### 2.2 使用 Django Debug Toolbar 风格的工具

**创建查询监控中间件**:

```python
# backend/app/middleware/query_monitor.py
from fastapi import Request
from starlette.middleware.base import BaseHTTPMiddleware
import time
import logging

logger = logging.getLogger(__name__)

class QueryMonitorMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        from sqlalchemy import event
        from sqlalchemy.engine import Engine
        
        query_count = 0
        query_times = []
        
        def count_queries(conn, cursor, statement, parameters, context, executemany):
            nonlocal query_count
            query_count += 1
            start = time.time()
            return start
        
        def log_query_time(conn, cursor, statement, parameters, context, executemany, start):
            elapsed = (time.time() - start) * 1000
            query_times.append(elapsed)
            if elapsed > 100:  # 慢查询警告
                logger.warning(f"Slow query ({elapsed:.2f}ms): {statement[:200]}")
        
        # 注册事件监听器
        event.listen(Engine, "before_cursor_execute", count_queries)
        event.listen(Engine, "after_cursor_execute", log_query_time)
        
        try:
            response = await call_next(request)
            
            # 在响应头中添加查询统计信息
            response.headers["X-Query-Count"] = str(query_count)
            response.headers["X-Total-Query-Time"] = f"{sum(query_times):.2f}ms"
            
            # 记录到日志
            if query_count > 10:  # 如果查询次数过多，记录警告
                logger.warning(
                    f"{request.url.path} - {query_count} queries, "
                    f"total time: {sum(query_times):.2f}ms"
                )
            
            return response
        finally:
            # 移除事件监听器
            event.remove(Engine, "before_cursor_execute", count_queries)
            event.remove(Engine, "after_cursor_execute", log_query_time)
```

**使用中间件**:

```python
# backend/main.py
from app.middleware.query_monitor import QueryMonitorMiddleware

app.add_middleware(QueryMonitorMiddleware)
```

**检查响应头**:

```bash
# 查看响应头中的查询统计
curl -I "http://localhost:8000/my-applications" \
  -H "Cookie: session_id=xxx"

# 应该看到：
# X-Query-Count: 2  # 优化后应该是 2-5 次
# X-Total-Query-Time: 45.23ms
```

**优化目标**:
- 用户申请列表：从 N+1 次查询减少到 2-3 次
- 任务申请列表：从 N+1 次查询减少到 2-3 次
- 任务列表：保持 1-2 次查询

---

## 3. 索引使用验证

### 3.1 使用 EXPLAIN ANALYZE

**这是最重要的检查方法！**

#### 检查任务列表查询

```sql
-- 1. 基础查询（应该使用 ix_tasks_status_created_id）
EXPLAIN ANALYZE
SELECT * FROM tasks 
WHERE status = 'open' 
ORDER BY created_at DESC, id DESC
LIMIT 20;

-- 预期结果：
-- Limit  (cost=0.43..15.23 rows=20 width=xxx) (actual time=0.123..0.456 rows=20 loops=1)
--   ->  Index Scan using ix_tasks_status_created_id on tasks  (cost=0.43..xxx rows=xxx) (actual time=0.120..0.450 rows=20 loops=1)
--         Index Cond: (status = 'open'::text)
-- Planning Time: 0.123 ms
-- Execution Time: 0.567 ms
```

**关键检查点**:
- ✅ 应该看到 `Index Scan using ix_tasks_status_created_id`
- ❌ 不应该看到 `Seq Scan`（全表扫描）
- ✅ `Execution Time` 应该 < 10ms

#### 检查任务类型+地点筛选

```sql
-- 2. 组合筛选查询（应该使用 ix_tasks_type_location_status）
EXPLAIN ANALYZE
SELECT * FROM tasks 
WHERE status = 'open' 
  AND task_type = 'delivery'
  AND location = 'London'
ORDER BY created_at DESC
LIMIT 20;

-- 预期结果：
-- Limit  (cost=0.43..15.23 rows=20 width=xxx) (actual time=0.123..0.456 rows=20 loops=1)
--   ->  Index Scan using ix_tasks_type_location_status on tasks  (cost=0.43..xxx rows=xxx) (actual time=0.120..0.450 rows=20 loops=1)
--         Index Cond: ((task_type = 'delivery'::text) AND (location = 'London'::text) AND (status = 'open'::text))
```

#### 检查用户任务查询

```sql
-- 3. 用户发布的任务（应该使用 ix_tasks_poster_status_created）
EXPLAIN ANALYZE
SELECT * FROM tasks 
WHERE poster_id = '12345678'
  AND status = 'open'
ORDER BY created_at DESC
LIMIT 20;

-- 预期结果：
-- Limit  (cost=0.43..15.23 rows=20 width=xxx) (actual time=0.123..0.456 rows=20 loops=1)
--   ->  Index Scan using ix_tasks_poster_status_created on tasks  (cost=0.43..xxx rows=xxx) (actual time=0.120..0.450 rows=20 loops=1)
--         Index Cond: ((poster_id = '12345678'::text) AND (status = 'open'::text))
```

#### 检查关键词搜索

```sql
-- 4. pg_trgm 搜索（应该使用 idx_tasks_title_trgm）
EXPLAIN ANALYZE
SELECT * FROM tasks 
WHERE status = 'open'
  AND similarity(title, 'delivery') > 0.2
ORDER BY similarity(title, 'delivery') DESC, created_at DESC
LIMIT 20;

-- 预期结果：
-- Limit  (cost=0.43..15.23 rows=20 width=xxx) (actual time=0.123..0.456 rows=20 loops=1)
--   ->  Bitmap Index Scan using idx_tasks_title_trgm on tasks  (cost=0.43..xxx rows=xxx) (actual time=0.120..0.450 rows=20 loops=1)
--         Index Cond: (similarity((title)::text, 'delivery'::text) > '0.2'::double precision)
```

#### 检查对话消息查询

```sql
-- 5. 对话消息查询（应该使用 ix_messages_conversation_created）
EXPLAIN ANALYZE
SELECT * FROM messages 
WHERE conversation_key = '12345678-87654321'
ORDER BY created_at ASC
LIMIT 50;

-- 预期结果：
-- Limit  (cost=0.43..15.23 rows=50 width=xxx) (actual time=0.123..0.456 rows=50 loops=1)
--   ->  Index Scan using ix_messages_conversation_created on messages  (cost=0.43..xxx rows=xxx) (actual time=0.120..0.450 rows=50 loops=1)
--         Index Cond: (conversation_key = '12345678-87654321'::text)
```

### 3.2 查看索引使用统计

```sql
-- 查看所有索引的使用情况
SELECT 
    schemaname,
    tablename,
    indexname,
    idx_scan as index_scans,  -- 索引扫描次数
    idx_tup_read as tuples_read,  -- 读取的元组数
    idx_tup_fetch as tuples_fetched  -- 获取的元组数
FROM pg_stat_user_indexes
WHERE tablename IN ('tasks', 'task_applications', 'messages', 'notifications')
ORDER BY idx_scan DESC;

-- 检查点：
-- 1. 新创建的索引应该有 idx_scan > 0（表示被使用过）
-- 2. 如果 idx_scan = 0，说明索引可能没有被使用，需要检查查询条件
```

### 3.3 检查未使用的索引

```sql
-- 查找可能未使用的索引（idx_scan = 0 且创建时间 > 1 天）
SELECT 
    schemaname,
    tablename,
    indexname,
    idx_scan,
    pg_size_pretty(pg_relation_size(indexrelid)) as index_size
FROM pg_stat_user_indexes
WHERE idx_scan = 0
  AND schemaname = 'public'
  AND tablename IN ('tasks', 'task_applications', 'messages', 'notifications')
ORDER BY pg_relation_size(indexrelid) DESC;

-- 注意：新创建的索引可能需要一些时间才会被使用
-- 如果索引创建后 1 周内都没有被使用，可能需要检查：
-- 1. 查询条件是否与索引匹配
-- 2. 索引列顺序是否正确
-- 3. 是否需要调整查询语句
```

---

## 4. 缓存效果检查

### 4.1 检查缓存命中率

**添加缓存统计**:

```python
# backend/app/redis_cache.py
import redis
from typing import Optional
import logging

logger = logging.getLogger(__name__)

class CacheStats:
    def __init__(self):
        self.hits = 0
        self.misses = 0
    
    @property
    def hit_rate(self):
        total = self.hits + self.misses
        return (self.hits / total * 100) if total > 0 else 0

# 全局统计对象
cache_stats = CacheStats()

def get_tasks_list_with_stats(cache_params):
    """获取任务列表（带统计）"""
    cache_key = get_tasks_cache_key(**cache_params)
    
    cached = redis_client.get(cache_key)
    if cached:
        cache_stats.hits += 1
        return json.loads(cached)
    else:
        cache_stats.misses += 1
        return None

def get_cache_stats():
    """获取缓存统计信息"""
    return {
        "hits": cache_stats.hits,
        "misses": cache_stats.misses,
        "hit_rate": f"{cache_stats.hit_rate:.2f}%",
        "total_requests": cache_stats.hits + cache_stats.misses
    }
```

**添加统计接口**:

```python
# backend/app/async_routers.py
@async_router.get("/admin/cache-stats")
async def get_cache_stats():
    """获取缓存统计信息（仅管理员）"""
    from app.redis_cache import get_cache_stats
    return get_cache_stats()
```

**检查方法**:

```bash
# 1. 触发一些请求（模拟正常使用）
for i in {1..100}; do
  curl "http://localhost:8000/tasks?limit=20&sort_by=latest" > /dev/null
done

# 2. 查看缓存统计
curl "http://localhost:8000/admin/cache-stats"

# 预期结果：
# {
#   "hits": 85,
#   "misses": 15,
#   "hit_rate": "85.00%",
#   "total_requests": 100
# }
```

**优化目标**:
- 缓存命中率应该 > 70%
- 如果命中率 < 50%，需要检查：
  - 缓存键设计是否合理
  - 缓存 TTL 是否太短
  - 是否有频繁的缓存失效

### 4.2 检查 Redis 内存使用

```bash
# 连接到 Redis
redis-cli

# 查看内存使用
INFO memory

# 查看 key 数量
DBSIZE

# 查看特定模式的 key 数量
KEYS "tasks:list:*" | wc -l
KEYS "tasks:count:*" | wc -l

# 查看 key 大小
MEMORY USAGE "tasks:list:v1:abc123"
```

**优化目标**:
- Redis 内存使用应该 < 100MB（取决于数据量）
- 如果内存使用过高，需要检查：
  - 是否有缓存泄漏（key 没有过期）
  - 缓存数据是否过大
  - 是否需要调整 TTL

### 4.3 检查缓存失效

```python
# 测试缓存失效逻辑
# 1. 创建一个任务
POST /tasks
# 2. 立即查询任务列表（应该看到新任务）
GET /tasks
# 3. 检查缓存是否已失效
# 4. 再次查询（应该从数据库重新加载）
GET /tasks
```

---

## 5. 功能正确性检查

### 5.1 分页正确性

**检查游标分页**:

```python
# 测试脚本
import requests

base_url = "http://localhost:8000"
session_id = "your_session_id"

# 1. 获取第一页
response1 = requests.get(
    f"{base_url}/tasks",
    params={"limit": 20, "sort_by": "latest", "cursor": None},
    headers={"Cookie": f"session_id={session_id}"}
)
data1 = response1.json()
tasks1 = data1["items"]
next_cursor = data1.get("next_cursor")

# 2. 获取第二页
response2 = requests.get(
    f"{base_url}/tasks",
    params={"limit": 20, "sort_by": "latest", "cursor": next_cursor},
    headers={"Cookie": f"session_id={session_id}"}
)
data2 = response2.json()
tasks2 = data2["items"]

# 3. 检查是否有重复
task_ids1 = {task["id"] for task in tasks1}
task_ids2 = {task["id"] for task in tasks2}
duplicates = task_ids1 & task_ids2

if duplicates:
    print(f"❌ 发现重复任务: {duplicates}")
else:
    print("✅ 没有重复任务")

# 4. 检查是否有遗漏（通过总数验证）
total = data1.get("total") or (len(tasks1) + len(tasks2))
if total and len(task_ids1 | task_ids2) > total:
    print(f"❌ 任务数量异常: 期望最多 {total}，实际 {len(task_ids1 | task_ids2)}")
else:
    print("✅ 任务数量正常")
```

### 5.2 数据一致性

**检查列表和总数是否一致**:

```python
# 测试脚本
import requests

def test_tasks_consistency():
    """测试任务列表和总数的一致性"""
    base_url = "http://localhost:8000"
    
    # 获取任务列表和总数
    response = requests.get(
        f"{base_url}/tasks",
        params={"limit": 100, "sort_by": "latest"}
    )
    data = response.json()
    
    tasks = data.get("tasks", [])
    total = data.get("total", 0)
    
    # 检查
    if len(tasks) > total:
        print(f"❌ 列表数量 ({len(tasks)}) 大于总数 ({total})")
        return False
    
    # 检查是否有重复
    task_ids = [task["id"] for task in tasks]
    if len(task_ids) != len(set(task_ids)):
        print(f"❌ 发现重复任务 ID")
        return False
    
    print(f"✅ 数据一致性检查通过: {len(tasks)}/{total}")
    return True
```

---

## 6. 自动化检查脚本

### 6.1 完整的检查脚本

```python
# scripts/check_optimization.py
#!/usr/bin/env python3
"""
优化效果检查脚本
"""
import asyncio
import time
import requests
from typing import Dict, List
import json

class OptimizationChecker:
    def __init__(self, base_url: str = "http://localhost:8000"):
        self.base_url = base_url
        self.results = {}
    
    def check_response_time(self, endpoint: str, params: Dict = None) -> Dict:
        """检查响应时间"""
        start = time.time()
        try:
            response = requests.get(f"{self.base_url}{endpoint}", params=params, timeout=10)
            elapsed = (time.time() - start) * 1000  # 毫秒
            
            return {
                "endpoint": endpoint,
                "status_code": response.status_code,
                "elapsed_ms": elapsed,
                "success": response.status_code == 200
            }
        except Exception as e:
            return {
                "endpoint": endpoint,
                "error": str(e),
                "success": False
            }
    
    def check_query_count(self, endpoint: str, params: Dict = None) -> Dict:
        """检查查询次数（通过响应头）"""
        try:
            response = requests.get(f"{self.base_url}{endpoint}", params=params, timeout=10)
            query_count = int(response.headers.get("X-Query-Count", 0))
            
            return {
                "endpoint": endpoint,
                "query_count": query_count,
                "success": query_count <= 5  # 期望 <= 5 次查询
            }
        except Exception as e:
            return {
                "endpoint": endpoint,
                "error": str(e),
                "success": False
            }
    
    def check_cache_hit_rate(self) -> Dict:
        """检查缓存命中率"""
        try:
            response = requests.get(f"{self.base_url}/admin/cache-stats", timeout=10)
            stats = response.json()
            
            hit_rate = float(stats.get("hit_rate", "0%").replace("%", ""))
            
            return {
                "hit_rate": hit_rate,
                "hits": stats.get("hits", 0),
                "misses": stats.get("misses", 0),
                "success": hit_rate >= 70  # 期望 >= 70%
            }
        except Exception as e:
            return {
                "error": str(e),
                "success": False
            }
    
    def run_all_checks(self):
        """运行所有检查"""
        print("🔍 开始优化效果检查...\n")
        
        # 1. 响应时间检查
        print("1. 检查响应时间...")
        endpoints = [
            ("/tasks", {"limit": 20, "sort_by": "latest"}),
            ("/tasks", {"limit": 20, "task_type": "delivery", "sort_by": "latest"}),
            ("/my-applications", {}),
        ]
        
        for endpoint, params in endpoints:
            result = self.check_response_time(endpoint, params)
            self.results[f"response_time_{endpoint}"] = result
            status = "✅" if result.get("success") and result.get("elapsed_ms", 0) < 200 else "❌"
            print(f"  {status} {endpoint}: {result.get('elapsed_ms', 0):.2f}ms")
        
        # 2. 查询次数检查
        print("\n2. 检查查询次数...")
        for endpoint, params in endpoints:
            result = self.check_query_count(endpoint, params)
            self.results[f"query_count_{endpoint}"] = result
            status = "✅" if result.get("success") else "❌"
            print(f"  {status} {endpoint}: {result.get('query_count', 0)} 次查询")
        
        # 3. 缓存命中率检查
        print("\n3. 检查缓存命中率...")
        result = self.check_cache_hit_rate()
        self.results["cache_hit_rate"] = result
        status = "✅" if result.get("success") else "❌"
        print(f"  {status} 缓存命中率: {result.get('hit_rate', 0):.2f}%")
        
        # 4. 总结
        print("\n📊 检查总结:")
        total_checks = len(self.results)
        passed_checks = sum(1 for r in self.results.values() if r.get("success"))
        print(f"  通过: {passed_checks}/{total_checks}")
        
        if passed_checks == total_checks:
            print("  ✅ 所有检查通过！")
        else:
            print("  ⚠️  部分检查未通过，请查看详细信息")
        
        return self.results

if __name__ == "__main__":
    checker = OptimizationChecker()
    results = checker.run_all_checks()
    
    # 保存结果到文件
    with open("optimization_check_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print("\n📄 详细结果已保存到 optimization_check_results.json")
```

**运行检查脚本**:

```bash
# 安装依赖
pip install requests

# 运行检查
python scripts/check_optimization.py
```

---

## 7. 持续监控

### 7.1 设置性能监控告警

**使用 Prometheus + Grafana**:

```python
# backend/app/middleware/metrics.py
from prometheus_client import Counter, Histogram, Gauge
import time

# 定义指标
request_count = Counter('http_requests_total', 'Total HTTP requests', ['method', 'endpoint', 'status'])
request_duration = Histogram('http_request_duration_seconds', 'HTTP request duration', ['method', 'endpoint'])
query_count = Histogram('db_queries_total', 'Database queries per request', ['endpoint'])
cache_hits = Counter('cache_hits_total', 'Cache hits', ['cache_type'])
cache_misses = Counter('cache_misses_total', 'Cache misses', ['cache_type'])

class MetricsMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        start_time = time.time()
        
        response = await call_next(request)
        
        # 记录指标
        elapsed = time.time() - start_time
        request_count.labels(
            method=request.method,
            endpoint=request.url.path,
            status=response.status_code
        ).inc()
        request_duration.labels(
            method=request.method,
            endpoint=request.url.path
        ).observe(elapsed)
        
        # 从响应头获取查询次数
        query_count_val = int(response.headers.get("X-Query-Count", 0))
        query_count.labels(endpoint=request.url.path).observe(query_count_val)
        
        return response
```

### 7.2 设置告警规则

```yaml
# prometheus/alerts.yml
groups:
  - name: performance_alerts
    rules:
      - alert: HighResponseTime
        expr: http_request_duration_seconds{quantile="0.95"} > 0.2
        for: 5m
        annotations:
          summary: "响应时间过高"
          description: "{{ $labels.endpoint }} 的 P95 响应时间超过 200ms"
      
      - alert: TooManyQueries
        expr: db_queries_total{quantile="0.95"} > 10
        for: 5m
        annotations:
          summary: "查询次数过多"
          description: "{{ $labels.endpoint }} 的查询次数超过 10 次"
      
      - alert: LowCacheHitRate
        expr: rate(cache_hits_total[5m]) / (rate(cache_hits_total[5m]) + rate(cache_misses_total[5m])) < 0.7
        for: 10m
        annotations:
          summary: "缓存命中率过低"
          description: "缓存命中率低于 70%"
```

---

## 8. 检查清单总结

### 优化前准备

- [ ] 记录优化前的性能指标（响应时间、查询次数、缓存命中率）
- [ ] 备份数据库
- [ ] 准备回滚方案

### 优化后检查

- [ ] **性能指标**
  - [ ] 响应时间降低 50% 以上
  - [ ] P95 响应时间 < 200ms
  - [ ] 数据库查询时间降低 50% 以上
  
- [ ] **N+1 查询**
  - [ ] 用户申请列表：查询次数 <= 3
  - [ ] 任务申请列表：查询次数 <= 3
  - [ ] 任务列表：查询次数 <= 2
  
- [ ] **索引使用**
  - [ ] 所有关键查询都使用了索引（EXPLAIN ANALYZE）
  - [ ] 没有出现 Seq Scan（全表扫描）
  - [ ] 索引使用统计显示新索引被使用
  
- [ ] **缓存效果**
  - [ ] 缓存命中率 >= 70%
  - [ ] Redis 内存使用正常
  - [ ] 缓存失效逻辑正确
  
- [ ] **功能正确性**
  - [ ] 分页数据不重复、不遗漏
  - [ ] 列表和总数一致
  - [ ] 搜索功能正常

### 持续监控

- [ ] 设置性能监控告警
- [ ] 定期检查慢查询日志
- [ ] 定期检查索引使用情况
- [ ] 定期检查缓存命中率

---

## 📝 检查报告模板

```markdown
# 优化效果检查报告

**检查日期**: 2025-01-27
**检查人员**: [姓名]

## 1. 性能指标对比

| 指标 | 优化前 | 优化后 | 提升 |
|------|--------|--------|------|
| 任务列表平均响应时间 | 300ms | 80ms | 3.75倍 |
| 任务列表 P95 响应时间 | 500ms | 150ms | 3.33倍 |
| 用户申请列表查询次数 | 15次 | 2次 | 7.5倍 |

## 2. 索引使用情况

- ✅ ix_tasks_status_created_id: 已使用，查询时间 < 10ms
- ✅ ix_tasks_type_location_status: 已使用，查询时间 < 15ms
- ✅ ix_messages_conversation_created: 已使用，查询时间 < 5ms

## 3. 缓存效果

- 缓存命中率: 85%
- Redis 内存使用: 45MB
- 缓存失效逻辑: 正常

## 4. 问题发现

- [列出发现的问题，如果有]

## 5. 建议

- [列出后续优化建议]
```

---

**重要提示**: 
- ⚠️ **必须使用 EXPLAIN ANALYZE 验证索引使用情况**，这是最可靠的检查方法
- ⚠️ **对比优化前后的性能指标**，确保优化真正有效
- ⚠️ **检查功能正确性**，确保优化没有引入 bug




